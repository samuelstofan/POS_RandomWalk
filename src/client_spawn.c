                 ...
                 uint32_t obstacle_seed, const char *obstacle_file)
{
    ...
    if (pid == 0) {
        ...
        char basebuf[32], obbuf[32], obdens[32], obseed[32];
        ...
        snprintf(obseed, sizeof(obseed), "%u", obstacle_seed);

        if (output_path && output_path[0]) {
            execl(server_path, server_path, sock_path, wbuf, hbuf, dbuf, rbuf, kbuf,
                  pu, pd, pl, pr, output_path, basebuf, obbuf, obdens, obseed,
                  obstacle_file ? obstacle_file : "", (char*)NULL);
        } else {
