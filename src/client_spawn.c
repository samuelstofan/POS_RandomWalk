                 ...
                 int base_replications)
{
    ...
    if (pid == 0) {
        (void)setsid();
        char wbuf[32], hbuf[32], dbuf[32], rbuf[32], kbuf[32], pu[32], pd[32], pl[32], pr[32], basebuf[32];
        ...
        if (output_path && output_path[0]) {
            execl(server_path, server_path, sock_path, wbuf, hbuf, dbuf, rbuf, kbuf, pu, pd, pl, pr, output_path, basebuf, (char*)NULL);
        } else {
